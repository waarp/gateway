...
.. http:put:: /api/rules/(string:rule_name)
   ...
   .. deprecated:: 0.5.0
      ...
      Les propriétés ``inPath`` et ``outPath`` de la requête ont été remplacées
      par les propriétés ``localDir`` et ``remoteDir``.

   .. deprecated:: 0.5.0

      La propriété ``workPath`` de la requête a été remplacée
   ...
   :reqjson string path: Le chemin d'identification de la règle. Sert à identifier
      ...
      chemin absolu.
   :reqjson string inPath: *Déprécié*. Le dossier de destination de la règle. Tous les fichiers
      transférés avec cette règle sont envoyés dans ce dossier. 
   :reqjson string outPath: *Déprécié*. Le dossier source de la règle. Tous les fichiers
      transférés avec cette règle sont récupérés depuis ce dossier. 
   :reqjson string workPath:  *Déprécié*.Le dossier temporaire de la règle. Tous les fichiers
