...
func getRange(req *http.Request) (progress uint64, err error) {
	...
	if matches == nil {
		err = fmt.Errorf("invalid Range value '%s' (only a single range-start is allowed)", head)
		return
	}
	progress, err = strconv.ParseUint(matches[0][1], 10, 64)
	if err != nil {
		err = fmt.Errorf("invalid range-start value '%s'", head)
		return
	}
	return
}
...
func getContentRange(headers http.Header) (progress uint64, filesize int64, err error) {
	...
	matches := reg.FindAllStringSubmatch(head, -1)
	if matches == nil {
		err = fmt.Errorf("invalid Content-Range value '%s'", head)
		return
	}

	if contRange := matches[0][1]; contRange != "*" {
		startEnd := strings.Split(contRange, "-")
		progress, err = strconv.ParseUint(startEnd[0], 10, 64)
		if err != nil {
			err = fmt.Errorf("invalid range-start value '%s'", matches[0][1])
			return
		}
	}
	if size := matches[0][2]; size != "*" {
		...
		if err != nil {
			err = fmt.Errorf("invalid size value '%s'", matches[0][2])
		}
		if s >= 0 {
			filesize = s
		}
