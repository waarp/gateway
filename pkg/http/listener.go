...
func (h *httpService) makeTLSConf() (*tls.Config, error) {
	...
	if err := h.db.Select(&certs).Where("owner_type=? AND owner_id=?", h.agent.TableName(),
		...
		return nil, fmt.Errorf("failed to retrieve server certificates")
	}
	...
	if len(tlsCerts) == 0 {
		...
		return nil, fmt.Errorf("could not find a valid certificate for HTTP server")
	}
...
func (h *httpService) listen() error {
	...
	if err != nil {
		...
		return fmt.Errorf("failed to retrieve HTTP server address")
	}
