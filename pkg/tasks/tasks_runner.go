...
func (p *Processor) runTask(task model.Task, taskInfo string) error {
	...
	if val, ok := runnable.(model.Validator); ok {
		if err2 := val.Validate(args); err2 != nil {
...
// setup contextualize and unmarshal the tasks arguments.
...
// replace replaces all the context variables (#varname#) in the tasks arguments
// by their context value.
func (p *Processor) replace(t *model.Task) ([]byte, error) {
	res := t.Args
	for key, f := range getReplacers() {
...
//nolint:funlen // can hardly be otherwise, or it should be designed another way
func getReplacers() map[string]replacer {
	return map[string]replacer{
