...
func (c *client) Start() error {
	...
	if err := c.start(); err != nil {
		...
		c.logger.Errorf("Failed to start the AMQP 0.9.1 client: %v", err)
...
func (c *client) start() error {
	...
	if err := utils.JSONConvert(c.dbClient.ProtoConfig, c.conf); err != nil {
		return fmt.Errorf("invalid AMQP 0.9.1 client config: %w", err)
	}

	if c.dbClient.LocalAddress.IsSet() {
		if _, err := net.ResolveTCPAddr("tcp", c.dbClient.LocalAddress.String()); err != nil {
			return fmt.Errorf("invalid AMQP 0.9.1 local address: %w", err)
		}
	}
...
func (c *client) Stop(ctx context.Context) error {
	...
	if err := pipeline.List.StopAllFromClient(ctx, c.dbClient.ID); err != nil {
		...
		return fmt.Errorf("failed to stop the AMQP 0.9.1 client: %w", err)
	}
...
func (c *client) InitTransfer(pip *pipeline.Pipeline) (protocol.TransferClient, *pipeline.Error) {
	...
	if err := utils.JSONConvert(pip.TransCtx.RemoteAgent.ProtoConfig, partConf); err != nil {
		return nil, pipeline.NewErrorWith(types.TeConnection, "invalid AMQP 0.9.1 partner config", err)
	}
