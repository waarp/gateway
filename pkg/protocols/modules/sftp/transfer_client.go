...
func (c *transferClient) makePartnerHostKeys(creds model.Credentials,
	...
	for _, cred := range creds {
		...
		if err != nil {
			c.pip.Logger.Warningf("Failed to parse the SFTP partner %q's hostkey %q: %v",
	...
	if len(hostKeys) == 0 {
		c.pip.Logger.Errorf("No valid hostkey found for partner %q", partner)
...
func (c *transferClient) openSSHConn() *pipeline.Error {
	...
	if dialErr != nil {
		c.pip.Logger.Errorf("Failed to connect to the SFTP partner: %v", dialErr)
	...
	if sshErr != nil {
		c.pip.Logger.Errorf("Failed to start the SSH session: %v", sshErr)
...
func (c *transferClient) startSFTPSession() *pipeline.Error {
	...
	if sftpErr != nil {
		c.pip.Logger.Errorf("Failed to start SFTP session: %v", sftpErr)
...
func (c *transferClient) Send(file protocol.SendFile) *pipeline.Error {
	...
	if _, statErr := c.sftpClient.Stat(parentDir); errors.Is(statErr, os.ErrNotExist) {
		...
		c.pip.Logger.Warningf("Failed to stat remote parent directory: %v", statErr)
	}
...
func (c *transferClient) endTransfer() (tErr *pipeline.Error) {
	...
	if c.sshClient != nil {
		if err := c.sshClient.Close(); err != nil {
			c.pip.Logger.Errorf("Failed to close SSH session: %v", err)
