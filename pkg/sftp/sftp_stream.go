...
// newSftpStream initializes a special kind of TransferStream tailored for
// the SFTP server. This constructor initializes a TransferStream, opens the
...
func newSftpStream(ctx context.Context, logger *log.Logger, db *database.DB,
	...
	s.Logger.Infof("Beginning transfer nb. %d", trans.ID)
...
func (s *sftpStream) ReadAt(p []byte, off int64) (int, error) {
	...
	if err != nil && !errors.Is(err, io.EOF) {
		...
		if err2 := s.Close(); err2 != nil {
			s.Logger.Warningf("An error occurred while closing the sftp stream: %v", err)
		}

		return n, modelToSFTP(s.transErr)
	}

	return n, err //nolint:wrapcheck // wrapping the error generates errors
}
