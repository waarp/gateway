...
func TestUserInit(t *testing.T) {
	...
	Convey("Given a database with no admins", t, func(c C) {
		db := database.TestDatabase(c)
	...
	Convey("Given an database with an admin", t, func(c C) {
		db := database.TestDatabase(c)
		So(db.DeleteAll(&User{}).Run(), ShouldBeNil)
