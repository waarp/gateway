...
var (
	// Clients is a map of all the currently active clients.
	Clients = map[string]Client{}
)

// Client is the type defining the signature which all clients
// constructor functions must satisfy.
type Client interface {
	proto.TransferManager
	InitTransfer(pip *Pipeline) (TransferClient, *types.TransferError)
	Start() error
	proto.Stopper
	proto.Statter
}
