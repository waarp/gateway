...
func ver0_14_0CreateTransfersView(db Actions) error {
		...
		As: `WITH transfers_as_history(id, owner, remote_transfer_id, is_server,
				...
				next_retry) AS (
					SELECT t.id, t.owner, t.remote_transfer_id,
						...
						t.start, t.stop, t.status, t.step, t.progress, t.task_number,
...
func ver0_14_0AddTransferStopUp(db Actions) error {
	...
	if err := db.AlterTable("transfers",
		AddColumn{Name: "stop", Type: DateTime{}, NotNull: false},
	); err != nil {
		return fmt.Errorf(`failed to add the transfer "stop" column: %w`, err)
	}

	return ver0_14_0CreateTransfersView(db)
}

func ver0_14_0AddTransferStopDown(db Actions) error {
	if err := db.DropView("normalized_transfers"); err != nil {
		return fmt.Errorf("failed to drop the transfers view: %w", err)
	}

	if err := db.AlterTable("transfers",
		DropColumn{Name: "stop"},
	); err != nil {
		return fmt.Errorf(`failed to drop the transfer "stop" column: %w`, err)
	}
