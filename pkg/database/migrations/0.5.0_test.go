...
func testVer0_5_0LocalAccountPasswordHash(eng *migration.Engine) {
	Convey("Given the 0.5.0 local account password renaming", func() {
		setupDatabaseUpTo(eng, ver0_5_0LocalAccountPasswordHash{})

		Convey("When applying the migration", func() {
			err := eng.Upgrade([]migration.Migration{{Script: ver0_5_0LocalAccountPasswordHash{}}})
			So(err, ShouldBeNil)

			Convey("Then it should have renamed the local account password column", func() {
				tableShouldNotHaveColumn(eng.DB, "local_accounts", "password")
				tableShouldHaveColumn(eng.DB, "local_accounts", "password_hash")
			})

			Convey("When reverting the migration", func() {
				err := eng.Downgrade([]migration.Migration{{Script: ver0_5_0LocalAccountPasswordHash{}}})
				So(err, ShouldBeNil)

				Convey("Then it should have reverted the changes", func() {
					tableShouldNotHaveColumn(eng.DB, "local_accounts", "password_hash")
					tableShouldHaveColumn(eng.DB, "local_accounts", "password")
				})
			})
		})
	})
...
func testVer0_5_0CertificatesRename(eng *migration.Engine, dbType string) {
	Convey("Given the 0.5.0 certificates table renaming", func() {
		setupDatabaseUpTo(eng, ver0_5_0CertificatesRename{})

		Convey("When applying the migration", func() {
			err := eng.Upgrade([]migration.Migration{{Script: ver0_5_0CertificatesRename{}}})
			So(err, ShouldBeNil)

			Convey("Then it should have renamed the certificates table", func() {
				So(doesTableExist(eng.DB, dbType, "certificates"), ShouldBeFalse)
				So(doesTableExist(eng.DB, dbType, "crypto_credentials"), ShouldBeTrue)
			})

			Convey("When reverting the migration", func() {
				err := eng.Downgrade([]migration.Migration{{Script: ver0_5_0CertificatesRename{}}})
				So(err, ShouldBeNil)

				Convey("Then it should have reverted the changes", func() {
					So(doesTableExist(eng.DB, dbType, "crypto_credentials"), ShouldBeFalse)
					So(doesTableExist(eng.DB, dbType, "certificates"), ShouldBeTrue)
				})
			})
		})
	})
