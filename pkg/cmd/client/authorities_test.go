			...
			t.Run("When executing the command", func(t *testing.T) {
				...
				assert.Equal(t,
					expectedOutput(t, authorities,
						...
						`    -Public Key Info`,
						`      -Algorithm: RSA`,
						`      -Public Value: 30 82 01 0A 02 82 01 01 00 D8 CE C0 41... (270 bytes total)`,
						`    -Fingerprints`,
						`      -SHA-256: 7B 03 62 47 C8 58 F0 80 E6 41 2D AB 12 A1 02 EA`,
						`                3A 66 21 0A 2F AF 04 53 31 61 B1 7C BE 43 C2 8A`,
						`      -SHA-1: 80 37 EA 8B 17 F7 FC 59 0A 04 4D CB DF 88 5F 27 BE 14 FA 31`,
						`    -Signature`,
						`      -Algorithm: SHA256-RSA`,
						`      -Public Value: 4D D6 2E 9C 0C 9C EB 72 D1 41 D3 F5 F9... (256 bytes total)`,
						`    -Serial Number: 0F 9B 36 4C 7E 99 7D 95 F8 30 F6 16 32 1A 54 A5`,
						`    -Key Usages: Digital Signature, Key Encipherment, Certificate Sign`,
