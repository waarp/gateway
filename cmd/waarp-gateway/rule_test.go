...
func ruleInfoString(r *models.OutRule) string {
	way := "receive"
	if r.IsSend {
		way = "send"
	}
	...
	taskStr := func(tasks []models.Task) string {
		str := ""
		for i, t := range tasks {
			prefix := "    ├─Command "
			if i == len(tasks)-1 {
